...
const AlertDetail = ({ alertData, machineData, reasons }) => {
  ...
  return (
    <Container fluid>
      <div
        className="py-4 d-flex flex-column justify-content-start align-items-start"
        style={{ borderBottom: "1px solid #dcddde" }}
      >
        <h3 className="mb-0 text-muted fw-normal">ALERT ID {alertData.id}</h3>
        <h5 className="text-muted fw-normal">
          Detected at{" "}
          {moment(alertData.detectionTime).format("YYYY-MM-DD HH:mm:ss")}
        </h5>
      </div>
      <Row className="my-3">
        <Col>
      ...
      <Row>
        <Col sm={4}>
          ...
          >
            ...
            {actions.map((act, ind) => (
              <option value={act.id} key={ind + "act"}>
                {act.name}
              </option>
            ))}
          </Form.Select>
